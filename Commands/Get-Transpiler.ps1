...
function Get-Transpiler
{
    ...
    begin {
        $TranspilerPattern = '\.psx\.ps1$','^PipeScript\p{P}Transpiler\p{P}(?!(?>format|types|tests)\p{P})','^psx\p{P}'
        ...
        function ConvertToExtension($toExtension) {
            ...
            $extCmd =
                ...
                elseif ($in -is [IO.FileInfo]) {
                    if ($in.LastWriteTime -gt $script:TranspilersFileTimes[$in.Fullname]) {
                        $script:TranspilersFileTimes[$in.Fullname] = $in.LastWriteTime
                        $script:TranspilersFromFiles[$in.Fullname] = 
                            $ExecutionContext.SessionState.InvokeCommand.GetCommand($in.fullname, 'ExternalScript,Application')
                        $script:TranspilersFromFiles[$in.Fullname]
                    } elseif ($script:TranspilersFromFiles[$in.Fullname])  {
                        return $script:TranspilersFromFiles[$in.Fullname]
                    }                    
                }
                else {
        ...
        if (-not $script:Transpilers)
        {
            ...
            $script:Transpilers =
                ...
                #region Find Transpiler in Loaded Commands                
